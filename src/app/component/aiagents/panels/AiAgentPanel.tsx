...
// --- API IMPORTS (unchanged from your store) ---
import {
  fetchAIReports,
  DailyAIReport,
 ...
 * ──────────────────────────────────────────────────────────────────────
 * AGENT IDENTITY
 * Change the name or the avatar any time — every part of the panel
 * (launcher bubble, header, message avatars, empty state) reads from
 * this one object. Drop the image in /public and point avatarUrl at it.
 * ──────────────────────────────────────────────────────────────────────
 * Mount this once near the root of your app (e.g. app/layout.tsx, inside
 * <body>) so the bubble follows the user across every page:
 *   <AIAgentPanel />
 */
const AGENT = {
  name: "Kartik",
  ...
  return (
    <div
      className={`relative shrink-0 overflow-hidden rounded-full bg-white ${
        ring ? "ring-2 ring-white/80 shadow-lg" : ""
      }`}
...
export default function AIAgentPanel() {
  ...
  // Load data lazily — only once the panel is actually opened, since this
  // component stays mounted (and otherwise idle) on every page.
  useEffect(() => {
  ...
  // ========================================================================
  // FORMATTING & REPORT RENDERING (behaviour unchanged from original)
  // ========================================================================
  ...
  const renderReportRow = (report: DailyAIReport) => {
    ...
    return (
      <button
        ...
        className={`group relative w-full overflow-hidden cursor-pointer rounded-2xl p-4 text-left transition-all ${
          isSelected
    ...
    <>
      {messages.length === 0 && !isLoadingMessages && (
        <div className="mx-auto flex h-full max-w-sm flex-col items-center justify-center text-center">
          ...
          <h2 className="mt-4 text-lg font-black text-gray-900">How can I help?</h2>
          <p className="mt-1.5 text-sm font-medium leading-relaxed text-gray-500">
            I'm your {AGENT.role.toLowerCase()}. Ask me to check tasks, assign work, pull reports, or explain what's
            happening in your CRM.
          </p>
        </div>
      )}
          ...
          return (
            ...
            >
              <div className={`flex max-w-[85%] gap-2.5 ${isUser ? "flex-row-reverse" : "flex-row"}`}>
                <div className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full shadow-sm ${isUser ? "bg-gray-900" : ""}`}>
                  {isUser ? <User size={15} className="text-white" /> : <AgentAvatar size={32} />}
                </div>
                <div
...
return (
    <>
      {/* Backdrop — only behind the expanded workspace view on desktop */}
      {isOpen && isExpanded && (
        <div
      ...
      {isOpen && (
        <div
          className={`fixed z-50 flex flex-col overflow-hidden bg-white shadow-2xl transition-all duration-300 animate-in fade-in zoom-in-95
            /* MOBILE: Always full screen, no rounded corners */
            top-0 left-0 right-0 bottom-0 h-[100dvh] w-full rounded-none
            
            /* DESKTOP: Expanded vs Compact states */
            ${
              isExpanded
                ...
                : "sm:bottom-24 sm:right-6 sm:top-auto sm:left-auto sm:translate-x-0 sm:translate-y-0 sm:h-[85vh] sm:max-h-[800px] sm:w-[400px] sm:rounded-[28px] "
            }
          `}
        >
          {/* connecting nub, only in the compact bubble state on desktop */}
          {!isExpanded && (
            <div className="absolute -bottom-2 right-8 hidden sm:block h-4 w-4 rotate-45 border-b border-r border-gray-200 bg-white" />
          )}

          {/* Header */}
          <div className={`relative shrink-0 bg-gradient-to-r from-[var(--color-primary)] to-[var(--color-primary-dark)] px-4 py-3.5 ${ isExpanded?"":"pt-5 "}  `}>
              ...
              <div className="flex shrink-0 items-center gap-1">
                {!isExpanded && (
                  <button
                    ...
                    className={`rounded-lg cursor-pointer p-1.5 transition-colors ${
                      historyOpen ? "bg-white/25 text-white" : "text-white/80 hover:bg-white/15 hover:text-white"
                    }`}
                    title="History"
                  >
                    <History size={16} />
                ...
                {/* Expand button strictly hidden on mobile via sm:inline-flex */}
                <button
            ...
            <div className="mt-3 flex rounded-xl bg-white/15 p-1">
              <button
                ...
                className={`flex-1 rounded-lg cursor-pointer py-1.5 text-xs font-bold transition-all ${
                  activeTab === "chat" ? "bg-white text-[var(--color-primary)] shadow-sm" : "text-white/85 hover:text-white"
                }`}
              >
                Live Chat
              </button>
              <button
                ...
                className={`flex-1 rounded-lg cursor-pointer py-1.5 text-xs font-bold transition-all ${
                  activeTab === "reports" ? "bg-white text-[var(--color-primary)] shadow-sm" : "text-white/85 hover:text-white"
                }`}
              >
                Daily Reports
              </button>
          ...
          {isExpanded ? (
            <div className="flex min-h-0 flex-1">
              {/* sidebar (desktop / tablet only — see the Expand button above) */}
      ...
      {!isExpanded && (
        <button
          onClick={togglePanel}
          /* Hide the floating bubble on mobile when panel is open (to avoid overlapping the full-screen view) */
          className={`fixed bottom-6 right-6 z-[60] cursor-pointer h-14 w-14 items-center justify-center rounded-full bg-white shadow-2xl ring-1 ring-black/5 transition-transform hover:scale-105 active:scale-95 pb-safe ${
